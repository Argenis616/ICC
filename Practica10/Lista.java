...
public class Lista<T> implements Iterable<T>{
    ...
    Lista<T> mergeSort(Lista<T> lista) {
		...
		Lista<T>.Nodo a = lista.cabeza;
		for(int b=0; b<lista.longitud/2; b++){
			x.agregaFinal(a.elemento);
			a = a.siguiente;
		}for(int c=lista.longitud/2; c<lista.longitud; c++){
			y.agregaFinal(a.elemento);
			a = a.siguiente;
		}
		x=Lista.mergeSort(x);
		y=Lista.mergeSort(y);
    ...
    public static <T extends Comparable<T>>
    Lista<T> merge(Lista<T>prim , Lista<T>segu) {
    Lista<T> ordenada =new Lista<T>();
    Lista<T>.Nodo a = prim.cabeza, b = segu.cabeza;
    while(a!=null&&b!=null){
		if(a.elemento.compareTo(b.elemento)<0){
		ordenada.agregaFinal(a.elemento);
		a = a.siguiente;
		
	}else{
		ordenada.agregaFinal(b.elemento);
		b = b.siguiente;
	}
	}while(a!=null){
